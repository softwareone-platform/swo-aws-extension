...
def test_pre_step_skips_roles_deployed(
    ...
    with pytest.raises(SkipStepError):
        CRMTicketOrderFail(config).pre_step(context)


def test_pre_step_skips_not_partner_led(
    order_factory, order_parameters_factory, fulfillment_parameters_factory, config
):
    order = order_factory(
        order_parameters=order_parameters_factory(
...
def test_pre_step_skips_ticket_exists(
    ...
    order = order_factory(
        ...
        fulfillment_parameters=fulfillment_parameters_factory(
            customer_roles_deployed=CustomerRolesDeployed.NO_DEPLOYED.value,
            crm_order_failed_ticket_id="TICKET-123",
        ),
    )
    context = PurchaseContext.from_order_data(order)
...
def test_pre_step_proceeds_conditions_met(
    ...
    CRMTicketOrderFail(config).pre_step(context)  # act
...
def test_process_creates_service_request(
    ...
    CRMTicketOrderFail(config).process(mpt_client, context)  # act

    contact = get_formatted_technical_contact(context.order)
    expected_service_request = ServiceRequest(
        additional_info=ORDER_FAILED_TEMPLATE.additional_info,
        summary=ORDER_FAILED_TEMPLATE.summary.format(
        ...
        title=ORDER_FAILED_TEMPLATE.title,
    )
...
def test_process_sets_ticket_id_on_success(
    ...
    assert get_crm_order_failed_ticket_id(context.order) == "TICKET-123"
...
def test_process_logs_ticket_creation(
    ...
    assert "Order Failed ticket created with ID TICKET-123" in caplog.text
...
def test_process_raises_error_when_crm_fails(
    ...
    with pytest.raises(UnexpectedStopError) as error:
        CRMTicketOrderFail(config).process(mpt_client, context)

    assert "Error creating Order Failed ticket" in error.value.title
...
def test_post_step_updates_order(
    ...
    CRMTicketOrderFail(config).post_step(mpt_client, context)  # act
