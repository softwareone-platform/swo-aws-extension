...
def test_update_all_parameters_visibility_defaults(mocker, order_factory):
    order = order_factory()
    step = SetParametersVisibleStep()
    context = InitialAWSContext.from_order_data(order)
    context.order["parameters"][PARAM_PHASE_ORDERING] = step.process_parameters(
        order["parameters"][PARAM_PHASE_ORDERING],
        always_shown=[OrderParametersEnum.ACCOUNT_NAME],
        always_hidden=[OrderParametersEnum.ACCOUNT_ID],
    )

    parameter = get_ordering_parameter(context.order, OrderParametersEnum.ACCOUNT_NAME)
    assert parameter["constraints"]["hidden"] is False

    parameter = get_ordering_parameter(context.order, OrderParametersEnum.ACCOUNT_ID)
    assert parameter["constraints"]["hidden"] is True
