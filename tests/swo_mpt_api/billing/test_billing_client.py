...
def test_upload_journal(mpt_client, journal_data, tmp_path):
    ...
    with file_path.open("rb") as f:
        response = api.billing.journal.upload(journal_id, f)
...
def test_delete_journal(mpt_client):
    ...
    response = api.billing.journal.delete(journal_id)
...
def test_regenerate_journal(mpt_client, journal_data):
    ...
    response = api.billing.journal.regenerate(journal_id)
...
def test_accept_journal(mpt_client, journal_data):
    ...
    response = api.billing.journal.accept(journal_id)
...
def test_submit_journal(mpt_client, journal_data):
    ...
    response = api.billing.journal.submit(journal_id)
...
def test_inquire_journal(mpt_client, journal_data):
    ...
    response = api.billing.journal.inquire(journal_id)

    assert response == journal_data
