...
def test_wrap_http_error_http_error(mocker):
    ...
    with pytest.raises(AWSHttpError) as error:
        wrapped_func()

    assert str(error.value) == "500 - Internal Server Error"
...
def test_wrap_boto3_error_client_error(mocker):
    ...
    assert (
        str(error.value) == "AWS Client error. An error occurred (TestError) when calling the T"
        "estOperation operation: Test error"
    )
...
def test_wrap_boto3_error_boto_core_error(mocker):
    ...
    func.side_effect = botocore_exceptions.BotoCoreError()
    wrapped_func = wrap_boto3_error(func)

    with pytest.raises(AWSError) as error:
        wrapped_func()

    assert str(error.value) == "Boto3 SDK error: An unspecified error occurred"
...
def test_wrap_boto3_error_unexpected_error(mocker):
    ...
    assert "Unexpected error: Unexpected error" in str(error.value)
