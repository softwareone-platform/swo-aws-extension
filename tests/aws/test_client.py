...
def test_aws_client_list_accounts(mocker, config, aws_client_factory):
    def data_account_factory(status="ACTIVE", account_id="1234-1234-1234", arn="arn"):
        return {"Id": account_id, "Arn": arn, "Status": status}
    aws_client, mock_client = aws_client_factory(config, "test_account_id", "test_role_name")
    mock_client.list_accounts.side_effect = [
        {
            "Accounts": [
                data_account_factory(account_id=f"0000-{0:04}-{j:04}") for j in range(10)
            ],
            "NextToken": "token0",
        },
        {
            "Accounts": [
                data_account_factory(account_id=f"0000-{1:04}-{j:04}") for j in range(10)
            ],
        },
    ]
