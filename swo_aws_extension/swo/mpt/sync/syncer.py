...
def check_existing_subscription_items(mpt_client, subs_items, subscription, agreement_id):
    ...
    skus_to_add = [sku for sku in ItemSkus if sku not in purchased_skus]
    skus_to_delete = [sku for sku in purchased_skus if sku not in ItemSkus]
    if skus_to_add:
    ...
    for account in active_accounts:
        ...
        try:
            ...
            product_items = get_product_items_by_skus(
                mpt_client, agreement.get("product").get("id"), ItemSkus
            )
...
def sync_agreement_subscriptions(mpt_client, aws_client, agreement, *, dry_run=False):
    ...
    subscription_status_to_skip = [
        ...
        SubscriptionStatus.TERMINATING,
        SubscriptionStatus.CONFIGURING,
    ]
