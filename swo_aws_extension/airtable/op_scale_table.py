...
class OpScaleEntitlementsTable:
    """Airtable table for OpScale Entitlements."""
    ...
    def __init__(self):
        ...
        self._table_name = "OpScale Entitlements"
        self._table = api.table(base_id, self._table_name)

    def get_by_agreement_id(self, agreement_id: str) -> list[OpScaleRecord]:
        """Get list of record by agreement id."""
        records = self._table.all(
            formula=EQUAL(FIELD(OpScaleFields.AGREEMENT_ID.value), STR_VALUE(agreement_id))
        )
        if not records:
            return []
        return [OpScaleRecord.from_airtable_record(record) for record in records]

    def save(self, record: OpScaleRecord) -> OpScaleRecord:
        ...
        return OpScaleRecord.from_airtable_record(result)

    def update_status_and_usage_date(
        self, record: OpScaleRecord, status: str, last_usage_date: str
    ) -> OpScaleRecord:
