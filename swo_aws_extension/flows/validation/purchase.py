...
class InitializeItemStep(Step):
    def __call__(self, client: MPTClient, context: PurchaseContext, next_step):
        items = get_product_items_by_skus(
            client, context.order.get("product", {}).get("id", ""), [AWS_ITEM_SKU]
        )
        if not items:
            logger.error(
                f"{context.order_id} - Failed to get product items with sku {AWS_ITEM_SKU}"
            )
            return

        context.order["lines"] = [{"item": items[0], "quantity": 1}]
        next_step(client, context)
