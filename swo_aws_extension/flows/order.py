...
def is_termination_type_close_account(order):
    return get_termination_type_parameter(order) == TerminationParameterChoices.CLOSE_ACCOUNT
...
def is_termination_type_unlink_account(order):
    return get_termination_type_parameter(order) == TerminationParameterChoices.UNLINK_ACCOUNT
...
class OrderContext(BaseContext):
    ...
    def from_order(cls, order: dict) -> "OrderContext":
        return cls(
            order=order, product_id=order["product"]["id"]
        )
    ...
    def __str__(self):
        return (
            f"Context: {self.order_id} {self.order_type} - MPA: {self.mpa_account}"
        )
...
class CloseAccountContext(OrderContext):
    ...
    def __str__(self):
        return (
            super().__str__()+
            f" - CloseAccount - Closing AWS Accounts: "
