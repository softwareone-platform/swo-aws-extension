...
Seller to legal entity mapping: [`swo_aws_extension/swo/cco/seller_mapper.py`](../../swo_aws_extension/swo/cco/seller_mapper.py), loading [`seller_external_id_map.json`](../../swo_aws_extension/swo/cco/seller_external_id_map.json) from the same package. `SellerMapper.resolve` uses the Navision company code from the seller `attributes.navision.companyCode` when present, and otherwise maps the Marketplace seller `externalId` (for example `ES` or `ES_CPX`) through the JSON file. The seller address country is not used, because a country can have several legal entities (SWO_ES, CPX_ES, IP_ES) and Navision rejects contracts sent to the wrong company
